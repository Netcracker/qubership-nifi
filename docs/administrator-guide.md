...
| NIFI_NEW_SENSITIVE_KEY                               | N                                  | <br/><br/><br/>              | Key used for encrypting sensitive properties in the NiFi configuration (flow.json.gz). Either this variable or file `/tmp/nifi-sens-key/key` must be non-empty. Must be at least 12 characters long. Do not change this value after the initial deployment; otherwise, an error will occur during startup. |
...
| NiFi sensitive key                 | File      | /tmp/nifi-sens-key/key                           | Contains the key used for encrypting sensitive properties in the NiFi configuration. File-based alternative to `NIFI_NEW_SENSITIVE_KEY` env variable. Must be at least 12 characters long. Do not change this value after the initial deployment; otherwise, an error will occur during startup. Will be used only if `NIFI_NEW_SENSITIVE_KEY` is not set. |
