...
Refer to Apache NiFi
 [System Administrator’s Guide](https://nifi.apache.org/docs/nifi-docs/html/administration-guide.html)
 for details on standard features.
...
| CONSUL_URL                                           | Y                                  |                              | URL to access Consul service. If `NIFI_CONSUL_INT_FRAMEWORK` = `spring`, it must be in format: `<hostname>:<port>`. Otherwise, `<protocol>://<hostname>:<port>`.                                                                                                                                                       |
| CONSUL_CONFIG_JAVA_OPTIONS                           | N                                  |                              | A list of additional Java startup arguments for auxiliary application used for Consul integration. Recommended value: see [Consul app JVM benchmark](../dev/consul-app-benchmark/README.md#recommended-jvm-options).                                                                                                   |
...
| SENSITIVE_KEY                                        | N                                  |                              | Deprecated key used for encrypting sensitive properties in the NiFi configuration (flow.json.gz). Should be set only in case of upgrade from older release that previously worked with `SENSITIVE_KEY`. Replaced with `NIFI_NEW_SENSITIVE_KEY`.                                                                        |
| SENSITIVE_KEY_PATH                                   | N                                  |                              | The path to file with the deprecated key used for encrypting sensitive properties in the NiFi configuration (flow.json.gz). Used only if `SENSITIVE_KEY` is not set.                                                                                                                                                   |
| OLD_SENSITIVE_KEY                                    | N                                  |                              | Deprecated previous key used for encrypting sensitive properties in the NiFi configuration (flow.json.gz). Should be set only in case of upgrade from older release that had these values.                                                                                                                             |
| OLD_SENSITIVE_KEY_PATH                               | N                                  |                              | The path to file with the deprecated previous key used for encrypting sensitive properties in the NiFi configuration (flow.json.gz). Used only if `OLD_SENSITIVE_KEY` is not set.                                                                                                                                      |
| NIFI_NEW_SENSITIVE_KEY                               | N                                  |                              | Key used for encrypting sensitive properties in the NiFi configuration (flow.json.gz). Either this variable or file under path `NIFI_SENSITIVE_KEY_PATH` must be non-empty. Must be at least 12 characters long. Do not change this value after the initial deployment; otherwise, an error will occur during startup. |
| NIFI_SENSITIVE_KEY_PATH                              | N                                  |                              | The path to file with the key used for encrypting sensitive properties in the NiFi configuration (flow.json.gz). Either variable `NIFI_NEW_SENSITIVE_KEY` or file under path `NIFI_SENSITIVE_KEY_PATH` must be non-empty.                                                                                              |
...
| KEY_PASSWORD                                         | N                                  |                              | The key password for secret key stored in the keystore. May contain any printable characters except for `\` (backslash). If neither `KEY_PASSWORD` nor `NIFI_KEY_PASSWORD_PATH` is set, the keystore password is used.                                                                                                 |
| NIFI_KEY_PASSWORD_PATH                               | N                                  |                              | The path to file with the key password for secret key stored in the keystore. If neither `KEY_PASSWORD` nor `NIFI_KEY_PASSWORD_PATH` is set, the keystore password is used.                                                                                                                                            |
...
| KEYSTORE_PASSWORD                                    | N                                  |                              | The keystore password for the keystore. May contain any printable characters except for `\` (backslash). Either `KEYSTORE_PASSWORD` or `NIFI_KEYSTORE_PASSWORD_PATH` must be set, if `AUTH` = oidc or tls or ldap.                                                                                                     |
| NIFI_KEYSTORE_PASSWORD_PATH                          | N                                  |                              | The path to file with the keystore password for the keystore. Either `KEYSTORE_PASSWORD` or `NIFI_KEYSTORE_PASSWORD_PATH` must be set, if `AUTH` = oidc or tls or ldap.                                                                                                                                                |
...
| TRUSTSTORE_PASSWORD                                  | N                                  |                              | The truststore password. May contain any printable characters except for `\` (backslash). Either `TRUSTSTORE_PASSWORD` or `NIFI_TRUSTSTORE_PASSWORD_PATH` must be set, if `AUTH` = oidc or tls or ldap.                                                                                                                |
| NIFI_TRUSTSTORE_PASSWORD_PATH                        | N                                  |                              | The path to file with the truststore password. Either `TRUSTSTORE_PASSWORD` or `NIFI_TRUSTSTORE_PASSWORD_PATH` must be set, if `AUTH` = oidc or tls or ldap.                                                                                                                                                           |
...
| ZOOKEEPER_CLIENT_KEYSTORE_PASSWORD                   | N                                  |                              | Password for keystore specified in variable `ZOOKEEPER_CLIENT_KEYSTORE`. Either this variable or file `/tmp/zk-client-keystore/client-keystore-password` must be non-empty, if `ZOOKEEPER_CLIENT_KEYSTORE` is not empty. May contain any printable characters except for `\` (backslash), `&`.                         |
...
The qubership-nifi Docker image provides several predefined extension points that can be used to customize its behavior
 without significant changes to other parts of the image.
...
| Persistent Configuration               | Directory | /opt/nifi/nifi-current/persistent_conf           | Stores nifi configuration (flow.json.gz), users, access policies, Database Repository, NAR Repository, Assets, Connector Assets and Python extensions source directory. Estimated total volume size is about 1 GB by default: flow.json.gz size is limited by the nifi.flow.configuration.archive.max.storage property (default value 300 MB); users and access policies < 100 KB; Database Repository < 300 MB; NAR Repository < 100 MB (depends on the number and size of NARs, typically 100 KB - 10 MB per NAR); Assets and Connector Assets < 50 MB; Python extensions source directory < 10 MB. See Apache NiFi [System Administrator's Guide](https://nifi.apache.org/docs/nifi-docs/html/administration-guide.html) for details. |
...
| Server key password                    | File      | `$NIFI_KEY_PASSWORD_PATH`                        | Contains the key password for secret key stored in the server keystore. File-based alternative to `KEY_PASSWORD` env variable. Will be used only if `KEY_PASSWORD` is not set.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| Server keystore password               | File      | `$NIFI_KEYSTORE_PASSWORD_PATH`                   | Contains the server keystore password. File-based alternative to `KEYSTORE_PASSWORD` env variable. Will be used only if `KEYSTORE_PASSWORD` is not set.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| Truststore password                    | File      | `$NIFI_TRUSTSTORE_PASSWORD_PATH`                 | Contains the truststore password. File-based alternative to `TRUSTSTORE_PASSWORD` env variable. Will be used only if `TRUSTSTORE_PASSWORD` is not set.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| NiFi sensitive key                     | File      | `$NIFI_SENSITIVE_KEY_PATH`                       | Contains the key used for encrypting sensitive properties in the NiFi configuration. File-based alternative to `NIFI_NEW_SENSITIVE_KEY` env variable. Must be at least 12 characters long. Do not change this value after the initial deployment; otherwise, an error will occur during startup. Will be used only if `NIFI_NEW_SENSITIVE_KEY` is not set.                                                                                                                                                                                                                                                                                                                                                                               |
| ZooKeeper client keystore password     | File      | /tmp/zk-client-keystore/client-keystore-password | Contains the password for keystore specified in variable `ZOOKEEPER_CLIENT_KEYSTORE`. File-based alternative to `ZOOKEEPER_CLIENT_KEYSTORE_PASSWORD` env variable. Will be used only if `ZOOKEEPER_CLIENT_KEYSTORE_PASSWORD` is not set.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| Deprecated NiFi sensitive key          | File      | `$SENSITIVE_KEY_PATH`                            | Contains the deprecated key used for encrypting sensitive properties in the NiFi configuration. File-based alternative to `SENSITIVE_KEY` env variable. Will be used only if `SENSITIVE_KEY` is not set.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| Deprecated previous NiFi sensitive key | File      | `$OLD_SENSITIVE_KEY_PATH`                        | Contains the deprecated previous key used for encrypting sensitive properties in the NiFi configuration. File-based alternative to `OLD_SENSITIVE_KEY` env variable. Will be used only if `OLD_SENSITIVE_KEY` is not set.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
1. Setting the logging level for a specific package in Consul. The Consul property name must start
    with `logger.` followed by the package name. The value should be one of the logging levels supported by Logback:
    ALL, TRACE, DEBUG, INFO, WARN, ERROR, OFF. The property should be located in one of the following locations:
...
1. Wait until the update is propagated to qubership-nifi. This may take up to 1 minute.
...
1. The property should be located in one of the following locations:
    1. config/${NAMESPACE}/application
    1. config/${NAMESPACE}/qubership-nifi

To change NiFi properties:

1. Update the NiFi properties in Consul as needed.
1. Restart the qubership-nifi container.
...
1. Set the version to restore in the Consul parameter `nifi-restore-version` located in `config/${NAMESPACE}/qubership-nifi`.
   The parameter must contain the name of the archived configuration to restore from, using the format
   `<timestamp>_flow.json.gz` (for example, `20250115T120000+0000_flow.json.gz`).
   The list of archived configuration versions is printed in the logs during service startup. Maximum number of versions
   listed is controlled by the `NIFI_ARCHIVE_CONF_MAX_LIST` environment variable (default is 50).
1. Restart the qubership-nifi container.

On startup, qubership-nifi performs the following:

1. Checks if the `nifi-restore-version` parameter is set
1. If the parameter is set and the specified archive file does not exist, a warning is printed in the logs and normal
   startup continues using the current configuration.
1. If the parameter is set and the specified archive file exists, the current configuration is moved to the archive
...
1. Use Kubernetes Leases for leader election and ConfigMaps for cluster-wide state storage.
...
1. environment variable `NIFI_ZK_ROOT_NODE` - path to the root node on ZooKeeper. Must be defined to avoid conflicts
   with other applications or NiFi cluster instances.
1. environment variable `ZOOKEEPER_SSL_ENABLED` - set to `true`, if ZooKeeper access is secured with TLS.
1. environment variable `ZOOKEEPER_CLIENT_KEYSTORE` - path to the keystore with the private key and
   certificate used to access ZooKeeper. Set only if ZooKeeper is configured to use x509 authentication
   and `ZOOKEEPER_SSL_ENABLED = true`.
1. environment variable `ZOOKEEPER_CLIENT_KEYSTORE_TYPE` - type for the keystore specified by `ZOOKEEPER_CLIENT_KEYSTORE`.
   Set only if ZooKeeper is configured to use x509 authentication and `ZOOKEEPER_SSL_ENABLED = true`.
1. environment variable `ZOOKEEPER_CLIENT_KEYSTORE_PASSWORD` - password for the keystore specified by `ZOOKEEPER_CLIENT_KEYSTORE`.
   Set only if ZooKeeper is configured to use x509 authentication and `ZOOKEEPER_SSL_ENABLED = true`.
...
1. Secured (TLS) with anonymous access - set `ZOOKEEPER_ADDRESS`, `NIFI_ZK_ROOT_NODE`, `ZOOKEEPER_SSL_ENABLED = true`,
   and add trusted certificates for ZooKeeper into the`Trusted certificates` directory.
1. secured (TLS) with x509 authentication - set `ZOOKEEPER_ADDRESS`, `NIFI_ZK_ROOT_NODE`, `ZOOKEEPER_SSL_ENABLED = true`,
   `ZOOKEEPER_CLIENT_KEYSTORE`, `ZOOKEEPER_CLIENT_KEYSTORE_TYPE`, `ZOOKEEPER_CLIENT_KEYSTORE_PASSWORD`, add trusted
   certificates for ZooKeeper into the `Trusted certificates` directory, add the keystore referenced
   by `ZOOKEEPER_CLIENT_KEYSTORE`.

When using ZooKeeper, Apache NiFi creates ZNodes within the path specified by `NIFI_ZK_ROOT_NODE`, o it requires at
least Create, Read, and Write privileges for this path if it was created by another application.
For more information on Apache NiFi behavior regarding ZooKeeper, refer to the
...
To use Kubernetes provider you must create ServiceAccount and properly configure Roles for it (`<your-namespace>` in
the snippet below must be replaced with the actual namespace where the NiFi cluster is deployed):
...
After this is done you need to set `serviceAccountName = nifi-sa` in your Deployment/StatefulSet, as well as configure
the necessary environment variables:

1. environment variable `NIFI_CLUSTER_LEADER_ELECTION_IMPLEMENTATION = KubernetesLeaderElectionManager`.
1. environment variable `NIFI_STATE_MANAGEMENT_PROVIDER_CLUSTER = kubernetes-provider`.
1. environment variable `NIFI_CLUSTER_LEADER_ELECTION_KUBERNETES_LEASE_PREFIX` - prefix for Lease resources created
   by NiFi cluster nodes.
1. environment variable `NIFI_KUBERNETES_CONFIGMAP_NAME_PREFIX` - prefix for ConfigMap resources created
   by NiFi cluster nodes.

If you have an existing cluster with Apache NiFi 1.x and rely on Cluster State in your flows, you may need to migrate
state to the new state provider when switching to the Kubernetes ConfigMap State Provider.
To do this, set the `nifi.state.management.provider.cluster.previous` property in Consul to `zk-provider`, as well as
set up the necessary connection properties (e.g., environment variables `ZOOKEEPER_ADDRESS`, `NIFI_ZK_ROOT_NODE`).
...
Once migration is complete, you should unset the `nifi.state.management.provider.cluster.previous` property and remove
the environment variables necessary for the ZooKeeper connection.
