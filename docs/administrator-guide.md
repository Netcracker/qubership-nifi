...
| Persistent Configuration               | Directory | /opt/nifi/nifi-current/persistent_conf           | Stores nifi configuration (flow.json.gz), users, access policies, Database Repository, NAR Repository, Assets, Connector Assets and Python extensions source directory. Estimated total volume size is about 1 GB: flow.json.gz ~300 MB; users and access policies < 100 KB; Database Repository < 300 MB; NAR Repository < 100 MB (depends on the number and size of NARs, typically N x 100 KB - 10 MB per NAR); Assets and Connector Assets < 50 MB; Python extensions source directory < 10 MB. See Apache NiFi [System Administrator's Guide](https://nifi.apache.org/docs/nifi-docs/html/administration-guide.html) for details. |
