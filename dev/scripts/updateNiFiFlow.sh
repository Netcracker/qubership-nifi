...
do	
	...
	needUpdate=$(jq 'all(.. | objects | select(has("bundle")) | .bundle | objects; (has("group") and has("version") | not) or (.group != "org.apache.nifi" or (.version | test("^2\\.[0-9]+\\.[0-9]+$") | not)))' "$f") || handle_error "Error checking version in exported flow - $f"
	if [[ "$needUpdate" == "true" ]]; then
		listForUpdate+=("$exportFlow")
		echo "Flow - $f needs to be updated"
	fi
done
...
respCode=$(curl -sS -w '%{response_code}' -o ./proc_type_resp.json "$NIFI_CERT" "$NiFi_TARGET_URL/nifi-api/flow/processor-types")
