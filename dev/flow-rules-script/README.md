...
Rules are matched by `Name`, so re-running the script does not create duplicates.
A rule whose enforcement policy and configured properties already match the configuration is left unchanged. If that
rule is `DISABLED` and `VALID`, the script enables it. A rule that differs is disabled, updated, and enabled again.
The script prints what it created, updated, or skipped.

A rule that is `INVALID` stays `DISABLED`. The script still processes every other entry, then prints the names of
those rules and exits with a non-zero code.
...
- Every custom rule type in the configuration installed in the target NiFi (the
...
| Consul returns `404` because the key is not set                                     | The configuration file.                                                      |
...
| Consul returns any other status code, such as `403` for a rejected ACL token        | Neither: the script prints the response body and exits with a non-zero code. |
...
The `get_consul_token` function in `createFlowRules.sh` returns the token the script sends with the Consul
request. Its default implementation returns `CONSUL_ACL_TOKEN`. Replace the body to read the token from somewhere
else, such as a mounted secret:
...
A configuration that lists a name twice fails the run before any rule is created or updated.
...
Only the properties listed in the configuration are compared against an existing rule.
A property the entry leaves out keeps whatever value the rule has, so it never counts as a difference.
A sensitive property always counts as a difference, because NiFi never returns its value.

A property `value` must not be `null`.
A configuration with a `null` value fails the run before any rule is created or updated.
...
against the limits below before you run the script.
...
| `RestrictSourceProcessorRunSchedule` | The source processor (timer driven, with no incoming connection) has a Run Schedule above `40 millis`, or its type is in `Ignored Processor Types`. |

`Ignored Processor Types` lists processors that can run as a source and do little work while idle: they wait inside
`onTrigger` with a timeout above 20 ms, or they yield. For them, a Run Schedule of `0 sec` costs little CPU. Listeners
that return from `onTrigger` without waiting or yielding, such as `HandleHttpRequest`, `ListenTCP`, `ListenUDP`, and
`ListenSyslog`, are left out on purpose. At `0 sec`, NiFi calls them constantly and idle CPU usage goes up, so give
them a Run Schedule above `40 millis`.

To keep one of these rules at `Warn` while you fix the flows, write a copy of the configuration with
`"Policy": "Warn"` for that rule to the Consul key and run the script again.
...
The tests need NiFi 2.x: flow analysis rules arrived in Apache NiFi 2.0, so the `dev-tools-test` workflow runs
...
6. For each entry whose `Name` already exists:
   ...
   - The configured enforcement policy and properties are compared against the rule. One that matches on every
     configured value is left alone, except that a rule that is `DISABLED` but `VALID` is enabled, and a rule
     that is `DISABLED` and not `VALID` fails the run.
...
7. For each entry whose `Name` does not exist yet:
   ...
   - If the rule is not `VALID` (for example, because of an invalid property value), it is left disabled, the
